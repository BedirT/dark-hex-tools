...
class Node:
    ...
    def select_child(self, pi_det: typing.Dict[str, typing.Tuple[int, float]]) \
        ...
        actions = [
            action for action in actions if action in pi_det[info_state].keys()
        ]
    ...
    def expanded(self, pi_det: typing.Dict[str, typing.Tuple[int,
                                                             float]]) -> bool:
    ...
    def expand(self, pi_det: typing.Dict[str, typing.Tuple[int, float]])\
        ...
        actions_det = [
            action for action in actions_det
            if action not in self.children.keys()
        ]
        rand_action = np.random.choice(actions_det)
        self.children[rand_action] = Node(self.state.child(rand_action), self,
                                          self.k)
...
class SoISMCTS:
    ...
    def __init__(self, game, args):
        ...
        self.all_states = pyspiel_get_all_states.get_all_states(
            self.game, include_terminals=False,
    ...
    def determine(self, branching_factor: int) \
        ...
        pi = self.tabular_policy(
            ...
            branching_factor
        )  # ! The branching factor should be set to -1 for the
        # ! player.
        return pi
    ...
    def tabular_policy(all_states, strategy_profile, branching_factor):
        ...
        for state in all_states:
            ...
            tabular_policy[info_state] = list(
                zip(
                    legal_actions, strategy_profile[player_idx](
