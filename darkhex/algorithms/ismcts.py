...
class Node:
    ...
    def select_child(self,
        ...
        best_action = max(actions, key=lambda action: self.children[action].value())
    ...
    def expanded(self, det_actions: typing.Dict[str, typing.List[int]]) -> bool:
        ...
        return all(action in self.children.keys() for action in det_actions[self.info_state])
    ...
    def expand(self, 
        ...
        actions_w_det = [action for action in det_actions[self.info_state]
        ...
        if obs_player != self.player:
            if not self.children:
                ...
                self.children[0] = child # only one child; no need to store action
        ...
        assert all([key in cur_state.legal_actions() for key in self.children.keys()])
        return rand_action, self.children[rand_action]

    def _new_random_child(self,
                          actions_w_det: typing.List[int],
    ...
    def update(self, 
               reward: int, 
               det_actions: typing.Dict[str, typing.List[int]],) -> None:
...
class ISMCTS:
    ...
    def run(self, state):
        root = Node(state.information_state_string(),
                    state.current_player(), None, self.args.k)
        ...
        for _ in range(self.args.num_simulations):
            ...
            # Select 
            while not next_state.is_terminal() and node.expanded(self.det_actions):
                # if node is not terminal and fully expanded for determinization
                action, node = node.select_child(self.det_actions, self.obs_player)
                next_state = next_state.child(action)

            # Expand
            if not next_state.is_terminal():
                action, node = node.expand(next_state, self.det_actions, self.obs_player)
                next_state = next_state.child(action)
    ...
    def _determine(self, state, det_actions):
        ...
        if self.args.branching_factor < 0 or \
            ...
            det_actions[info_state] = np.random.choice(legal_actions, 
                                                    self.args.branching_factor,
                                                    replace=False)
