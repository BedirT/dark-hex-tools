...
def get_all_information_states(game: pyspiel.Game,
                               num_rows,
                               num_cols,
                               include_terminal_states=True) -> list:
...
def _get_all_info_states(state: pyspiel.State, info_states: dict,
                         state_data: dict, include_terminal_states) -> None:
...
if __name__ == "__main__":
    num_rows = 4
    num_cols = 3
    get_all_information_states(
        pyspiel.load_game(
            f"dark_hex_ir(num_rows={num_rows},num_cols={num_cols})"), num_rows,
        num_cols)
