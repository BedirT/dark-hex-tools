...
def test_so_ismcts_pom():
    ...
    for n in range(n_games):
        ...
        o_win_str = bar_indicator * (bar_size * num_wins[1] // (num_wins[1] + num_wins[0]))
        p_win_str = bar_indicator * (bar_size - len(o_win_str))
        print(f'{n+1}/{n_games}:' + ' ' * 10 + f'ISMCTS {chips_per_hand[0]}|{num_wins[0]} |{p_color}{p_win_str}{end_color}' +
              f'{neutral_color}{neutral_indicator}{end_color}' +
              f'{o_color}{o_win_str}{end_color}| {num_wins[1]}|{chips_per_hand[1]} Random Player', end="\r", flush=True)
    print()
