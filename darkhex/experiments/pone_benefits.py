...
def num_states_encountered():
    ...
    # No pone
    start = time.time()
    game = pyspiel.load_game(f"dark_hex_ir(num_rows={num_rows},num_cols={num_cols})") 
    info_states = get_all_information_states(game, num_rows, num_cols, include_terminal_states=True, get_data=False)
    ...
    plt.bar(x, y, width=.9, color=sns.color_palette("tab10")[:2], align='center')
