...
def print_init_board_middle(stdscr, num_cols, num_rows):
    ...
    extra_text = [
        'Player 1 (B) is played by the pONE agent',
        'Player 2 (W) is you, please make a move',
        'according to the given table indexes. Here', 'are the board indexes;'
    ]
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            stdscr.addstr(y + 2 + row, x + (col + 1) * 3 + spaces,
                          '{0: <3}'.format(cell))
            if col == num_cols - 1:  # last col
                stdscr.addstr(y + 2 + row, x + (col + 1) * 3 + 2 + spaces,
                              '\\' + pieces.C_PLAYER2)
            cell += 1
    y += row
    ...
    stdscr.addstr(
        y + 4, x,
        ' ' * (num_rows + 4) + '{0: <3}'.format(pieces.C_PLAYER1) * num_cols)
...
def text_box(stdscr, input_text, box_size, extra_notes=None):
    ...
    rectangle(stdscr, y - 2, w // 2 - (box_size // 2) - 3, y,
              w // 2 + (box_size // 2) + 3)
