...
$arg = shift @ARGV;
$rt3d2 = 0.866025403;
open INFILE, "template11.master.ps";
...
while ($filename = shift @files) {
    if ($filename =~ /(.*)\.hexdiag$/) {
        ...
        $labels = 0;  # modify in case of labels outside the board
        $dimY = 0;  $dimX = 0;
	$diagname = $1;
	...
	# assuming HexBoardBorders
	    ...
	    $miny = -(1.5*$dimX)*$HexRadius*$Scale - 2;
	    $maxy = (1.5)*$HexRadius*$Scale; 
	    if ($labels==1) {$maxy = $maxy + 5;}  #add a bit more for labels
	    }
...
##	&AddBoundingCoordinates($1,$2);
    ...
    #}
    elsif ($line =~ /DrawHexPLBoard/ ||
           $line =~ /HexLine/ ||
           $line =~ /HexCurve/ ||
	   $line =~ /(\S+)\s+(\S+).*[^s]LabelString/ ||
           $line =~ /(\S+)\s+(\S+).*ShadedHexagon/ ||
           $line =~ /HexBoardBorder/ ||
           $line =~ /HexBlackMarker/ ||
           $line =~ /HexBlackPiece/ ||
           $line =~ /HexBlackLabelledPiece/ ||
           $line =~ /HexWhiteMarker/ ||
           $line =~ /HexWhiteLabelledPiece/ ||
           $line =~ /HexWhitePiece/ ||
           $line =~ /HexDot/ ||
	   $line =~ /FontSelect/ ||
	   $line =~ /setgray/ ||
