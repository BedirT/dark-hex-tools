...
def traverse_the_game(board_state, 
                      info_states, 
                      player, 
                      opponent, 
                      num_cols, 
    ...
    while not valid_moves:
        valid_moves = True
        moves, probs, fill_randomly = get_moves(board_state, num_cols, num_rows, fill_randomly)
    ...
    for move in moves:
        ...
        if collusion_possible:
            ...
            if new_board not in info_states: 
                traverse_the_game(new_board, info_states, player, opponent, num_cols, num_rows, isomorphic, fill_randomly)
        # Generate the information state for a non-collusion.
        new_board = moves_and_boards[str(move)+player]
        if new_board not in info_states:
            traverse_the_game(new_board, info_states, player, opponent, num_cols, num_rows, isomorphic, fill_randomly)
        
def generate_information_states(num_cols,
                                num_rows,
                                player,
                                isomorphic,
                                board_state=None,
