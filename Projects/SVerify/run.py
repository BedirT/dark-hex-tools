...
def start_the_game(game_orig, p_order, S, p_color, o_color, player_turn):
    '''
    ...
    if player_turn == p_order:
        ...
        if game_orig.get_information_set(p_color) not in S:
            # p has no moves, the game is tied
            # S is incomplete)
            print('{}\n{}'.format(game_orig.get_information_set(p_color), game_orig.BOARD))
            game_orig.printBoard()
            incomplete = True
            return False
        prob = 1 / len(S[game_orig.get_information_set(p_color)])
        for action in S[game_orig.get_information_set(p_color)]:
            game = deepcopy(game_orig)
        ...
        pos_results = []
        for action in game_orig.valid_moves:
            game = deepcopy(game_orig)
...
def main(): 
    ...
    game = DarkHex(BOARD_SIZE=[strategy_dict['num_rows'], strategy_dict['num_cols']],
                    verbose=True)
